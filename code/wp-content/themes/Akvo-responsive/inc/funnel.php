...
<?php 
	...
	while( have_rows('section') ): the_row();
		...
		if( $i >= $section_count ){ $next_link = '#assess07';}
...
<section id="funnel-<?php _e( $i );?>" class="funelContainer hidden" style="<?php _e( $styles );?>">
	<div class="funnelContent">
		...
		<h1><?php the_sub_field('question');?></h1>
		...
		<ul class="<?php if( $buttons_count <=2 ){_e('inlined');}?>">
			...
			<li><a href="<?php _e( $next_link );?>" data-behaviour="fnl-nxt-btn" class="bouton"><?php the_sub_field('title')?></a></li>
			<?php endwhile;?>
		</ul>
	</div>
</section>		
<?php $i++; endwhile;?>	
<section id="assess07" class="funelContainer hidden" style="background-image:url('<?php the_field('form_bg');?>');">
	<div class="funnelContent">
